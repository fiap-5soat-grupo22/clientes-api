...
describe('CommonsService', () => {
  ...
  describe('queryFieldsObject success', () => {
    it('Deve retornar um objeto com as respectivas propriedades', async () => {
      expect(
        service.queryFieldsObject(
          'ativo=true,tipo=colaborador,conta=123234.12345,pais=França,nome=João da Silva',
        ),
      ).toStrictEqual({
        ativo: true,
        tipo: 'colaborador',
  ...
  describe('queryFieldsObject error typeOf', () => {
    it('Deve retornar um objeto nulo', async () => {
      expect(
        service.queryFieldsObject(12323 as unknown as string),
      ).toStrictEqual({});
    });
  });
